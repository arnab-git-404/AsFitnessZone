...
export default function MeasurementsPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (r.ok) {
                toastRef.current?.show({ severity: 'success', summary: 'Measurements saved! 📏' });
                setWeight('');
                setWaist('');
                setArms('');
                fetchMeasurements();
                ...
                const data = await r.json();
                toastRef.current?.show({ severity: 'error', summary: data.error || 'Failed to save' });
            }
        ...
        <div className="min-h-screen bg-background">
            ...
            <main className="container mx-auto px-4 py-8">
                <div className="max-w-4xl mx-auto">
                    <TabView activeIndex={activeTab} onTabChange={(e) => setActiveTab(e.index)} className="measurement-tabs">
                        <TabPanel header="Progress Chart">
                            <Card className="!border-border/50">
                                <div className="p-6">
                                    <h2 className="text-lg font-semibold flex items-center gap-2 mb-4">
                                        <TrendingUp className="h-5 w-5 text-primary" />Weight Progress
                                    </h2>
                                    {weightData.length < 2 ? (
                        ...
                        <TabPanel header="Log New">
                            <Card className="!border-border/50">
                                <div className="p-6">
                                    <h2 className="text-lg font-semibold mb-4">Log Body Measurements</h2>
                                    <form onSubmit={handleSubmit} className="space-y-4">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <InputTextarea id="notes" value={notes} onChange={e => setNotes(e.target.value)} rows={2} className="w-full" />
                                        </div>
                                        <Button type="submit" className="w-full bg-primary border-primary text-white" label={isSaving ? 'Saving...' : 'Save Measurements'} disabled={isSaving} />
                                    </form>
                                </div>
                            </Card>
                        </TabPanel>
...
function Field({ label, value, onChange }: { label: string; value: number | ''; onChange: (v: number | '') => void }) {
    return (
        <div className="space-y-1">
            ...
            <InputText type="number" value={String(value)} onChange={e => onChange(e.target.value ? Number(e.target.value) : '')} className="h-9 w-full" />
        </div>
    );
}
